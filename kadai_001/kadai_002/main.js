...
const timer = () => {
    ...
    const id = setInterval(() => {
        ...
        if (time <= 0) {
            ...
            timeup.style.display = 'block'; // タイムアップメッセージを
            typedfield.textContent = ""; // 入力済み文字列を表示
            untypedfield.textContent = "";// 未入力文字列を表示

            setTimeout(() => {
                gameOver(id); // ゲームオーバー処理を呼び出す
            }, 10); // 10ミリ秒後にゲームオーバー処理を実行
        }
    }, 1000);
};
